...
# -*- coding: utf-8 -*-
"""
BigWorld .animation writer (aligned to official grammar)

改进点：
- header 增加 loop/cognate/alpha/interpolation
- 尾部写入 markers 表
"""
...
def export_animation_file(filepath, animation_data):
    ...
    animation_data dict expected keys:
      ...
      - cognate: bool
      - alpha: bool
      - interpolation: int (0=linear,1=step,2=spline)
      - markers: optional list of { time: float, name: str }
    """
...
def _write_header(f, anim):
    ...
    Binary header layout (little-endian):
      ...
      - flags: 4 bytes
          ...
          [1] cognate: u8 (0/1)
          [2] alpha: u8 (0/1)
          [3] interpolation: u8 (0=linear,1=step,2=spline)
    """
    ...
    interp_mode = int(anim.get("interpolation", 0))  # 0=linear,1=step,2=spline
    f.write(struct.pack("<BBBB", loop_flag, cognate_flag, alpha_flag, interp_mode))
...
def _write_keyframes(f, keyframes, bones):
    ...
    for kf in keyframes:
        ...
        for bone in bones:
            ...
            tr = bt[bname]
            pos, rot, scl = tr.get("position"), tr.get("rotation"), tr.get("scale")
            ...
            for v in pos: write_f32(f, float(v))
            for v in rot: write_f32(f, float(v))
            for v in scl: write_f32(f, float(v))
