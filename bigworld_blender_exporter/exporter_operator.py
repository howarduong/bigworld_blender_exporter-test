...
class BIGWORLDEXPORTER_OT_export(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for idx, mat in enumerate(bpy.data.materials):
            ...
            textures_list = []
            if mat.node_tree:
                # 简单解析：所有 Image Texture 节点导出
                for n in mat.node_tree.nodes:
                    if n.type == 'TEX_IMAGE' and getattr(n, "image", None):
                        # 粗略判断color_space（法线节点名称包含“normal”可视为linear）
        ...
        for obj in context.selected_objects:
            ...
            visual_rel = write_visual_for_object(
                obj, primitives_meta, material_fx_map, tmp_res_root, rel_visual_dir="visuals"
            )
            # materialNames 来自 material_fx_map 的 name 列表（按slot顺序）
        ...
        # 可选：部署到项目 res（默认注释，谨慎开启）
        # import shutil
        # deploy_target = os.path.abspath(config.get("res_root","res"))
        # shutil.copytree(tmp_res_root, deploy_target, dirs_exist_ok=True)
        # LOG.info(f"已部署到 {deploy_target}")
