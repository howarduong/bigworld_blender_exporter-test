...
class BigWorldModelSettings(PropertyGroup):
    ...
    lod_distance_1: FloatProperty(
        name="LOD 1 Distance",
        description="LOD 1距离 (LOD 1 distance)",
        default=10.0,
        min=0.1
    )
    ...
    lod_distance_2: FloatProperty(
        name="LOD 2 Distance",
        description="LOD 2距离 (LOD 2 distance)",
        default=25.0,
        min=0.1
    )
    ...
    lod_distance_3: FloatProperty(
        name="LOD 3 Distance",
        description="LOD 3距离 (LOD 3 distance)",
        default=50.0,
        min=0.1
    )
...
class BigWorldAnimationSettings(PropertyGroup):
    ...
    loop_animation: BoolProperty(
        name="Loop Animation",
        description="循环动画 (Loop animation)",
        default=False
    )
...
class BigWorldMaterialSettings(PropertyGroup):
    ...
    material_type: EnumProperty(
        ...
        items=config.MATERIAL_TYPES,
        default='STANDARD'
    )
    
    shader_path: StringProperty(
        name="Shader Path",
        description="着色器路径 (Shader file path)",
        default=config.DEFAULT_SHADER
    )
    
    diffuse_texture: StringProperty(
        name="Diffuse Texture",
        description="漫反射贴图 (Diffuse texture)",
        default=""
    )
    
    normal_texture: StringProperty(
        name="Normal Texture",
        description="法线贴图 (Normal texture)",
        default=""
    )
    
    specular_texture: StringProperty(
        name="Specular Texture",
        description="高光贴图 (Specular texture)",
        default=""
    )
...
def register():
    ...
    bpy.utils.register_class(BigWorldModelSettings)
    bpy.utils.register_class(BigWorldAnimationSettings)
    bpy.utils.register_class(BigWorldMaterialSettings)

    bpy.types.Scene.bigworld_export = bpy.props.PointerProperty(type=BigWorldExportSettings)
    bpy.types.Object.bigworld_model = bpy.props.PointerProperty(type=BigWorldModelSettings)
    bpy.types.Action.bigworld_animation = bpy.props.PointerProperty(type=BigWorldAnimationSettings)
    bpy.types.Material.bigworld_material = bpy.props.PointerProperty(type=BigWorldMaterialSettings)
...
def unregister():
    ...
    bpy.utils.unregister_class(BigWorldModelSettings)
    bpy.utils.unregister_class(BigWorldAnimationSettings)
    bpy.utils.unregister_class(BigWorldMaterialSettings)
